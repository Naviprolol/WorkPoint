...
export class RequestsToAddComponent implements OnInit {
  ...
  sortOldToNew(): void {
    this.filteredCoworkings.sort((coworkingFirst: ICoworking, coworkingSecond: ICoworking) => moment(coworkingFirst.created_at).diff(moment(coworkingSecond.created_at)));
  }
